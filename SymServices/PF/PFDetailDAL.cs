...
order by SectionOrderNo ";
        ...
        /// </returns>
        private string[] InsertPFHeader(int FiscalYearDetailId, string PId, SqlConnection currConn, SqlTransaction transaction, ShampanIdentityVM auditvm, string TransType)
        {
            #region Variables
            string[] retResults = new string[6];
            retResults[0] = "Fail";//Success or Fail
            ...
            try
            {
                ...
                sqlText = @"insert into PFHeader (Code,[FiscalYearDetailId],[ProjectId],[EmployeePFValue],[EmployeerPFValue],[Post],
                            [Remarks],[IsActive],[IsArchive],[CreatedBy],[CreatedAt],[CreatedFrom],[LastUpdateBy],[LastUpdateAt],[LastUpdateFrom],BranchId,TransType) 
                            values (@Code ,@FiscalYearDetailId,@ProjectId,@EmployeePFValue,@EmployeerPFValue,@Post,@Remarks,@IsActive,@IsArchive,
                            @CreatedBy,@CreatedAt,@CreatedFrom,@LastUpdateBy,@LastUpdateAt,@LastUpdateFrom,@BranchId,@TransType)  
                          ";

                string deleteCurrent = @" DELETE FROM PFHeader  WHERE FiscalYearDetailId=@FiscalYearDetailId and ProjectId=@ProjectId and BranchId=@BranchId and TransType=@TransType";
        ...
        public string[] ImportExcelFile(string fullPath, string fileName, ShampanIdentityVM avm, SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null, int FYDId = 0, string PId = "", string TransType="")
        {
            #region Initializ
            string sqlText = "";
            int Id = 0;
            ...
            try
            {
                ...
                retResults = InsertPFHeader(FYDVM.Id, PId, currConn, transaction, avm, TransType);
